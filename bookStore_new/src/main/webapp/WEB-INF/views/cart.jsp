    ...
    <div class="cartProductList">
        <table class="cartProductTable">
            ...
            <tr >
                ...
                <td style="text-align: center">商品信息${orderItems.size()}</td>
                <td>单价</td>
                <td>数量</td>
                ...
                <tr orderItemId="${oi.id}" class="cartProductItemTR">
                    <td>
                        ...
                        <img class="cartProductImg"
                             src="${oi.bookNew.image}" referrerpolicy="no-referrer">
                    </td>
                  <td  style="text-align: center">
                        <div class="cartProductLinkOutDiv">
                            <a href="/showProduct?product_id=${oi.bookNew.id}"
                               class="cartProductLink">

                                <p>${oi.bookNew.bookname}</p>
                                <p>${oi.bookNew.author}</p>
                                <p>${oi.bookNew.publicsource}</p>
                            </a>
                         ...
                         <td>
                           <span class="cartProductItemOringalPrice">￥${oi.bookNew.price}</span>
                           <span class="cartProductItemPromotionPrice">￥${oi.bookNew.price}</span>

                       </td>
                       <td>

                           <div class="cartProductChangeNumberDiv">
                               <span hidden class="hidden orderItemStock " product_id="${oi.bookNew.id}">${oi.bookNew.salenumber}</span>
                               <span hidden class="hidden orderItemPromotePrice "
                                     product_id="${oi.bookNew.id}">${oi.bookNew.price}</span>
                               <a product_id="${oi.bookNew.id}" class="numberMinus" href="#nowhere">-</a>
                               <input style="width: 30px" product_id="${oi.bookNew.id}" orderItemId="${oi.id}" class="orderItemNumberSetting"
                                      autocomplete="off" value="${oi.number}">
                               <a stock="${oi.bookNew.storenumber}" product_id="${oi.bookNew.id}" class="numberPlus"
                                  href="#nowhere">+</a>
                           </div>

                       </td>
                       <td>
                               <span class="cartProductItemSmallSumPrice" orderItemId="${oi.id}" product_id="${oi.bookNew.id}">
                                       ${oi.bookNew.price*oi.number}
                               </span>

                       </td>
                       <td>
                           <a class="deleteOrderItem" orderItemId="${oi.id}" href="/deleteCartBook?book_id=${oi.bookNew.id}">删除</a>
                       </td>
                </tr>
            </c:forEach>
...
<div class="cartDiv">
    ...
    <div class="cartProductList">
        <table class="cartProductTable">
            <thead>
            <tr><th class="selectAndImage" >
                    <img style="margin-left: -90px" selectit="false" class="selectAllItem" src="img/fore/cartNotSelected.png">
                    全选

                </th>
                <th>商品${orderItems.size()}</th>
                <th>单价</th>
                <th>数量</th>
            ...
            <c:forEach items="${orderItems}" var="oi">
                <tr orderItemId="${oi.id}" class="cartProductItemTR">
                    <td>
                        <img  style="margin-bottom: 50px" selectit="false" orderItemId="${oi.id}" class="cartProductItemIfSelected"
                             src="img/fore/cartNotSelected.png">
                        <a style="display:none" href="#nowhere"><img src="img/fore/cartSelected.png"></a>
                        <img class="cartProductImg"
                             src="img/product/${oi.id}/1.jpg">
                    </td>
                    <td>
                        <div class="cartProductLinkOutDiv">
                            <a href="/showProduct?product_id=${oi.id}" class="cartProductLink">
                                <p>书名：${oi.bookname}</p>
                                <p>作者：${oi.author}</p>
                                <p>简介：${oi.bookinfo}</p>
                  ...
                  <td>
                        <span class="cartProductItemOringalPrice">￥${oi.price}</span>
                        <span class="cartProductItemPromotionPrice">￥${oi.price}</span>

                    </td>
                         <td>

                           <div class="cartProductChangeNumberDiv" >
                               <span class="hidden orderItemStock " product_id="${oi.id}">${oi.salenumber}</span>
                               <span class="hidden orderItemPromotePrice " product_id="${oi.id}">${oi.price}</span>
                               <a product_id="${oi.id}" class="numberMinus" href="#nowhere" style="width: 15px;margin-left: -21px">-</a>
                               <input style="width:20px;" product_id="${oi.id}" orderItemId="${oi.id}" class="orderItemNumberSetting"
                                      autocomplete="off" value="${oi.salenumber}">
                               <a style="width:15px;" stock="${oi.salenumber}" product_id="${oi.id}" class="numberPlus"
                                  href="#nowhere">+</a>
                           </div>

                       </td>
                       <td>
                               <span style="margin-left: 50px" class="cartProductItemSmallSumPrice" orderItemId="${oi.id}" product_id="${oi.id}">
                                       ${oi.price*oi.salenumber}
                               </span>

                       </td>
                       <td>
                           <a class="deleteOrderItem" orderItemId="${oi.id}" href="deleteCartBook?book_id=${oi.id}"style="color:#000;;">删除</a>
                       </td>
                </tr>
            </c:forEach>
            </tbody>
